...
function writeLaunchers(ns) {
  ["oneshot", "loop", "util"].forEach((dir) => ns.ls("home", dir).forEach((s) => ns.rm(s))), ((wrt) => [
...
async function gvnr(ns) {
  const refresh_delay = 1;
  const runLoop = async (is_first_start, timer) => (prettyLogs(ns), await prettyOverview(ns, timer), timer % 40 == 0 && await scriptLoop(ns, is_first_start), await util.slp(refresh_delay * 1e3), await runLoop(false, timer + refresh_delay));
  ns.tail(), ns.disableLog("ALL"), ns.tprintf(`${util.ansi.m}** ./gvnr.js **`), ns.atExit(() => (HOOKS[0].innerText = "", HOOKS[1].innerText = "")), // Clears the overview on exit to prevent stale data)
  await runLoop(true, 0);
}
async function Run(ns, path, params = [], props = "") {
  !ns.fileExists("run.js") && ns.write(
    `run.js`,
    [
      "/** @param ns {NS} */",
  ...
  return !run_pid ? (ns.tprintf(`${util.ansi.r}!! ${path} DID NOT RUN !!`), null) : (await ns.nextPortWrite(run_pid), ns.readPort(run_pid));
}
var util = {
...
async function is_Busy(ns) {
  return await Run(ns, "singularity.getCurrentWork", [""], "type") == "GRAFTING" || await Run(ns, "bladeburner.inBladeburner") && !!await Run(ns, "bladeburner.getCurrentAction", [""], "name");
}
function sGet(ns, m = /* @__PURE__ */ new Set(["home"])) {
...
async function ramUp(ns) {
  await Run(ns, "singularity.upgradeHomeRam") && (ns.tprintf(util.ansi.g + "Upgraded home ram"), true) && ramUp(ns);
}
async function coresUp(ns) {
  await Run(ns, "singularity.upgradeHomeCores") && (ns.tprintf(util.ansi.g + "Upgraded home cores"), true) && coresUp(ns);
}
...
async function darkwebShopping(ns) {
...
async function d43m0nD357r0y(ns, date = /* @__PURE__ */ new Date(), wd = "w0r1d_d43m0n") {
  (sGet(ns).includes(wd) && await Run(ns, "getHackingLevel") > await Run(ns, "getServerRequiredHackingLevel", [wd]) || ns.bladeburner.inBladeburner() && !ns.bladeburner.getNextBlackOp()) && (["installCounter.txt", "installAugsReason.txt", "installAugsLog.txt"].map((f) => ns.rm(`temp/${f}`)), ns.write(
    "report/nodeLog.txt",
...
function prettyLogs(ns) {
  ...
  const getSecDelta = (server) => secColour(
    Math.ceil(ns.getServerSecurityLevel(server) - ns.getServerMinSecurityLevel(server)).toString().padStart(3, " ")
  );
  ...
  ns.resizeTail(800, 1e3), ns.moveTail(CON.WIN.innerWidth - 1150, 0), ns.clearLog(), [
    ...[
...
async function prettyOverview(ns, timer) {
  ...
  [HOOKS[0].innerHTML, HOOKS[1].innerHTML] = [
    ...
    },
    {
      lines: [
        [`target:`, `${prsm_target}`],
        [`$/s:`, `$${ns.formatNumber(ns.getScriptIncome("loop/prsm.js", void 0))}`],
...
async function scriptLoop(ns, is_first_start) {
  await ["contracts.js", ...CON.ONESHOT_FUNCTIONS.map((s) => `oneshot/${s}.js`)].reduce(
    async (last, script) => (await last, is_first_start && ns.tprintf(`${util.ansi.y}starting ${script} `), await (async (runpid) => !!runpid ? (await ns.nextPortWrite(runpid), is_first_start && (await util.slp(70 * Math.random()), ns.tprintf(`${util.ansi.g}${script} passed init`))) : ns.tprintf(`${util.ansi.r} !!${script} DID NOT RUN!!`))(ns.run(script))),
    void 0
  ), CON.LOOP_FUNCTIONS.map((s) => `loop/${s}.js`).forEach((script) => !ns.isRunning(script) && ns.run(script)), is_first_start && (ns.print(`${util.ansi.m} Welcome to gnvr.js!`), ns.tprintf(`${util.ansi.g}*** Startup Complete *** `), await util.slp(1e3), ns.run("util/neofetch.js"));
}
async function graft(ns, g = ns.grafting) {
...
async function factWork(ns, s = ns.singularity) {
  ...
  !await is_Busy(ns) && !!target_faction && (s.stopAction(), ["field", "security", "hacking"].some(
    (job) => s.workForFaction(available_augs.includes(CON.TRP) ? "Daedalus" : target_faction, job, false)
  ));
}
async function donate(ns, s = ns.singularity) {
  const availableAugs2 = readyFiley(ns, "temp/availableAugs.txt");
  ...
  s.getFactionFavor(donatefaction) >= 150 * rep_multi && s.getFactionRep(donatefaction) < nfginfo.rep && s.donateToFaction(donatefaction, Math.max(nfginfo.cost, 1e11)) && ns.tprintf(
  ...
  availableAugs2.forEach(
    (aug) => aug.fact.name != CON.GANG_NAME && s.getFactionFavor(aug.fact.name) > 150 * rep_multi && aug.repdelta > 0 && s.donateToFaction(aug.fact.name, 1e11) && ns.tprintf(`${util.ansi.m}Donated $100B to ${aug.fact.name} `)
  );
}
async function installAugs(ns) {
  ...
  const timeout_log = `timeout - $${ns.formatNumber(ns.getServerMoneyAvailable("home"))} /$${ns.formatNumber(lowest_price)}, multi x${Math.floor(+readyFiley(ns, "temp/priceRatio.txt"))} - ${timestamp}`;
  ...
  const checkTimeout = () => time_since_last_aug > 18e5 && lowest_price > ns.getServerMoneyAvailable("home") && (writeLog(timeout_log), true);
  const writeLog = (log) => ns.write("temp/installAugsReason.txt", `installAugs #${1 + +readyFiley(ns, "temp/installCounter.txt")}: ${log}`, "w");
  ...
  !await is_Busy(ns) && !!bought_augs.length && (checkTimeout() || checkFavour()) && (ns.write("temp/installCounter.txt", (1 + +readyFiley(ns, "temp/installCounter.txt")).toString(), "w"), ns.write("report/installAugsLog.txt", readyFiley(ns, "temp/installAugsReason.txt") + "\n", "a"), await Run(ns, "singularity.installAugmentations", ["rset.js"]));
}
function buyAugs(ns, s = ns.singularity) {
  const odd_factions = ["Bladeburners", "Church of the Machine God"];
  const availableAugs2 = readyFiley(ns, "temp/availableAugs.txt");
  const timeStamp = () => ns.write("temp/lastAugTime.txt", "" + Date.now(), "w");
  const termPrint = (aug) => ns.tprintf(
    `${util.ansi.m}Purchased ${util.ansi.c}${aug.aug}${util.ansi.m} from ${aug.fact.name} for $${ns.formatNumber(aug.price)}`
  );
  const termPrintNFG = (faction) => ns.tprintf(
...
async function ownedAugs(ns) {
  const wrt = async (file, data) => await Run(ns, "write", [file, JSON.stringify(data), "w"]);
  !await Run(ns, "fileExists", ["temp/installCounter.txt"]) && wrt("temp/installCounter.txt", 0), wrt(
    "temp/boughtAugs.txt",
  ...
  ), wrt("temp/ownedAugs.txt", await Run(ns, "singularity.getOwnedAugmentations", [1])), wrt("temp/installedAugs.txt", await Run(ns, "singularity.getOwnedAugmentations", [0])), wrt("temp/nfgInfo.txt", {
    cost: await Run(ns, "singularity.getAugmentationPrice", [CON.NFG]),
    rep: await Run(ns, "singularity.getAugmentationRepReq", [CON.NFG])
  });
}
async function availableAugs(ns, s = ns.singularity) {
  ...
  const aug_object = ns.getPlayer().factions.filter((faction) => !forbidden_factions.includes(faction)).flatMap(
...
function hacknetShindigs(ns, h = ns.hacknet) {
  ...
  const upMoney = () => ns.getServerMaxMoney(moneytargetserver) < 1e13 && h.spendHashes("Increase Maximum Money", moneytargetserver) && upMoney();
  const downSec = () => ns.getServerMinSecurityLevel(sectargetserver) > 1 && h.spendHashes("Reduce Minimum Security", sectargetserver) && downSec();
...
async function steves(ns) {
  const [s, b, g] = [ns.sleeve, ns.bladeburner, ns.gang];
...
async function bburner(ns) {
  const [s, b] = [ns.singularity, ns.bladeburner];
  const goTrain = async () => (await Run(ns, "singularity.stopAction"), await Run(ns, "singularity.travelToCity", ["Sector-12"]), await Run(ns, "singularity.gymWorkout", [
    "Powerhouse Gym",
  ...
  const doOp = async (op) => !op ? d43m0nD357r0y(ns) : (([a, c]) => a + c > 1.8)(b.getActionEstimatedSuccessChance("BlackOps", b.getNextBlackOp().name)) && !await is_Busy(ns) && (s.stopAction(), b.startAction("BlackOps", b.getNextBlackOp().name));
  b.joinBladeburnerDivision(), !b.inBladeburner() ? await goTrain() : (upSkill(), await doOp(b.getNextBlackOp()?.name));
}
async function stan(ns, s = ns.stanek) {
  ...
  !!target && (spare_threads > 0 && !isNaN(target.numCharge) ? ns.exec("chrg.js", "home", spare_threads, target.x, target.y) : ns.print("no threads! skipping...")), ns.writePort(ns.pid, ""), await util.slp(1e4), await stan(ns);
}
...
async function prsm(ns) {
  ...
  async function runLoop(run_p_obj) {
    ...
    ns.clearPort(ns.pid), ns.writePort(ns.pid, target.hostname), await util.slp(batch_delay), await runLoop(batch_complete_p_obj);
  }
  await runLoop(await Run(ns, "getPlayer"));
}
async function neofetch(ns) {
  const blk = "\u2588";
...
var rn = Math.random;
var tget = (ns) => ns.getRunningScript().tailProperties ?? ns.exit();
var names = ["Wickes", "Updog", "Mikasa", "Snuffles", "Boris", "Gnasher", "Doug", "Chester"];
var bark = (ns, bark2) => ns.print(bark2);
function checkBounds(ns, t) {
  let returnval = { x: 0, y: 0 }, x = t.x, y = t.y, w = t.width, h = t.height, otherprogs = ns.ps().map((p) => ns.getRunningScript(p.pid)?.tailProperties).filter((p) => p);
  otherprogs.forEach(
    (prog) => {
      if (prog.x > x + w)
        returnval.x - 50;
...
function step(ns, zoomievalue, target, bool = 1, randbool = true) {
  let screenratio = win.innerHeight / win.innerWidth, x = tget(ns).x, y = tget(ns).y;
  if (target.x > x) {
...
function poop(ns, x, y, pooppid) {
  ns.write("poop.js", poopScript, "w"), pooppid = ns.run("poop.js"), ns.tail(pooppid), ns.resizeTail(150, 100, pooppid), ns.moveTail(x, y, pooppid);
}
async function downDog(ns, prevposx, prevposy) {
  ...
  while (true) {
    ...
    barkval === void 0 ? ascii.forEach((line) => ns.print(line)) : bark(ns, barkval.bark);
