    ...
    while (true) {
        ...
        if (time % n == 0 && !ns.args[0]) {
            ...
            for (const script of scriptstorun) {
                ...
                if (firststart) ns.tprint(`${c.g}${script} passed init`);
                await (ns.sleep(rundelay)) // This sleep is just so it looks cool when it prints :)
        ...
        const percColour = (perc) => {
            (perc < 33) ? perc = `${c.r}${perc}`: (perc < 66) ? perc = `${c.y}${perc}` : (perc < 85) ? perc = `${c.c}${perc}` : perc = `${c.g}${perc}`;
            return perc.padStart(11, " ");
        }
        let sec = (sec) => {
            (sec < 0) ? sec = `${c.g}${sec}`: (sec < 66) ? sec = `${c.y}${sec}` : sec = `${c.r}${sec}`;
            return sec.padStart(8, " ")
        }
        ...
        // Report	
        if (firststart) {
            ns.tprint(`${c.g}*** Startup Complete ***`);
            await ns.sleep(1000)
        }
        let spacer = " | "
        ...
        for (const aug of installedaugs)
            if (aug != "NeuroFlux Governor") otheraugs++
