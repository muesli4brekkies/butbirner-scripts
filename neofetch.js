...
const distroAscii = (ns) => {
	const dateFormat = (date) => `${Math.floor(date / (60 * 24))} days, ${Math.floor(date / ((60)) % 24)} hours, ${Math.floor(date % 60)} mins`,
		...
		packages = `Packages: ${w}${ns.ls("home").length} (bitpkg)`,
		shell = `Shell: ${w}bit-sh 6.9`,
		resolution = `Resolution: ${w}${win.innerWidth} x ${win.innerHeight}`, // Costs a lot of ram
	...
	`${r}    ${g}FFFFFFFF\\${r}.......${g}TTTTTTTT\\      ${g}${title}`,
	`${r}    ${g}FF \\_____|${r}:~:~:~${g}\\__TT \\__|     ${g}${dashes}`,
